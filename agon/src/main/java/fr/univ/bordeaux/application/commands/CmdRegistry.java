...
/**
 * register command to access it
 * without command limits
 * only one {@link CmdRegistry} can be created at the same time
 * (modular)
 */
public class CmdRegistry {
  ...
  /**
   * register a {@link ICmd} to be used from an
   * identifier for later access by {@link String}
   * the String is lowercased by the function
   * @param name command name
   * @param cmd class associated with the name
   * (many names for same command is possible)
   */
  ...
  /**
   * can return corresponding command if found
   * else return null ({@link Optional} show that the result can be null explicitly)
