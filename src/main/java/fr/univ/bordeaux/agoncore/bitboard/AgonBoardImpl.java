...
public class AgonBoardImpl implements AgonBoard {
   ...
   * <p>This constructor parses the text generated by {@link #toTextList()}
   * and automatically deduces any pieces waiting for relocation.</p>
   *
      ...
      for (int c = 0; c < 11; c++) {
        ...
        if (this.validZoneMask.isSet(idx)) {
          if (charIndex < cleanLine.length()) {
            ...
            switch (piece) {
              case 'Q': this.whiteQueen.setBit(idx, 1L); break;
              case 'q': this.blackQueen.setBit(idx, 1L); break;
              case 'O': this.whitePawns.setBit(idx, 1L); break;
              case 'X': this.blackPawns.setBit(idx, 1L); break;
              case '.': break;
            }
            charIndex++;
          }
