...
public interface AgonBoard extends RestrictedAgonBoard {
   ...
   *
   * @return A {@link List} of strings, where each element is a formatted description of a move.
   *     Returns an empty list if no moves have been performed yet.
   */
  ...
  /**
   * Checks if a specific player has any pieces currently captured that must be returned to the
   * board.
   *
   * @param color The {@link Color} of the player whose relocation status is being checked.
   * @return {@code true} if the player has at least one piece (Queen or Pawn) pending relocation;
   *     {@code false} if all the player's pieces are already on the board.
   */
