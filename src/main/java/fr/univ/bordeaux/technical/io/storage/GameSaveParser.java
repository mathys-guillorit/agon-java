 ...
 * (via {@link SaveParserState}) to sequentially process the distinct sections of a save file.
 * As the parser reads through the file, it dynamically changes its parsing behavior depending on
 * the current section header being processed ({@code [settings]}, {@code [game]}, or {@code [history]}).
 *
 * <p>Data aggregation is delegated to a {@link GameSaveBuilder}, which acts as a central repository
 * for the extracted data and ultimately validates the file's structural integrity before instantiating
 * the final object.
 */
public class GameSaveParser extends AbstractFileParser<GameSaveData> {
  ...
  /**
   * Processes a list of pre-cleaned strings (with comments and empty lines removed)
   * to construct a complete {@link GameSaveData} instance.
   *
   * <p>This method iterates through the lines sequentially. If a section header is detected
   * (e.g., {@code [game]}), it delegates to {@link #switchState(String, GameSaveBuilder)} to update
   * the active parsing state. Otherwise, it delegates the line parsing to the {@code currentState}.
   * If an individual line fails to parse, the error is logged and the parser continues. If the
   * final construction fails due to missing critical sections, it returns {@code null}.
   *
   * @param cleanLines The list of formatted string lines from the save file.
   * @return A fully populated {@link GameSaveData} object, or {@code null} if the file is structurally invalid.
   */
  @Override
   ...
   *
   * <p>This method also interacts with the {@link GameSaveBuilder} to flag the presence
   * of essential file sections, ensuring that missing or fully truncated sections are
   * caught during the final validation phase.
   *
