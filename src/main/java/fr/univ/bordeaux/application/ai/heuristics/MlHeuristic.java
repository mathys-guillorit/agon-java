 ...
 *
 * <p>Implements a variant of the PUCT (Predictor + UCB) algorithm popularized by AlphaZero.
 * Instead of relying solely on random simulation statistics (rollouts), this heuristic uses
 * a neural network to predict the win probability of a given state, while preserving the
 * exploration factor (C) of the UCT formula.</p>
 */
public class MlHeuristic implements MctsSelectionHeuristic {
    ...
    /**
     * Evaluates a child node relative to its parent using the PUCT algorithm combined with the TensorFlow model prediction.
     *
     ...
     * @return The calculated selection score combining the model's win probability and the exploration bonus.
     */
    @Override
    ...
    /**
     * Calls the in-memory TensorFlow model to obtain a win probability prediction for the current board state.
     *
     * @param features The array of 6 numerical features extracted from the game board.
     * @return The predicted probability of winning, ranging between 0.0 and 1.0. Returns 0.5 as a fallback if the model fails.
     */
    ...
    /**
     * Translates the board state into an array of six numerical features required by the TensorFlow model.
     *
     * @param board The current state of the game board.
     * @return A float array containing the calculated features: white queen distance, black queen distance,
     * guard position advantage, capture advantage, white mobility, and black mobility.
     */
