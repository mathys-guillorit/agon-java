 ...
 * <p>
 * Verifies the mathematical calculations for the hexagonal grid, mouse event handling,
 * rendering logic branches, and piece visualization rules.
 */
public class HexagonCanvasTest {
    ...
    void testPixelToHex_MathAndOutOfBounds() {
        RestrictedAgonBoard dummyBoard = (RestrictedAgonBoard) Proxy.newProxyInstance(
        ...
        assertDoesNotThrow(() -> {
            canvas.getOnMousePressed().handle(createMouseEvent(MouseEvent.MOUSE_PRESSED, 10000, 10000));
        });
    ...
    void testTakeImmediateSnapshot_WithExceptions() {
        ...
        RestrictedAgonBoard exceptionBoard = (RestrictedAgonBoard) Proxy.newProxyInstance(
                RestrictedAgonBoard.class.getClassLoader(),
                new Class<?>[]{RestrictedAgonBoard.class},
                (proxy, method, args) -> {
    ...
    void testMouseInteractionWithBoard() {
        ...
        double centerY = canvas.getHeight() / 2;
        assertDoesNotThrow(() -> {
            canvas.getOnMouseDragged().handle(createMouseEvent(MouseEvent.MOUSE_DRAGGED, centerX, centerY));
            canvas.getOnMouseReleased().handle(createMouseEvent(MouseEvent.MOUSE_RELEASED, centerX, centerY));
        });
    }
    ...
    void testPixelToHex_BoundaryConditions() throws Exception {
        Method method = HexagonCanvas.class.getDeclaredMethod("pixelToHex", double.class, double.class);
        method.setAccessible(true);
        String centerHex = (String) method.invoke(canvas, canvas.getWidth() / 2, canvas.getHeight() / 2);
    ...
    void testTakeImmediateSnapshot_WithActualPiece() {
        ...
        RestrictedAgonBoard boardWithPiece = new RestrictedAgonBoard() {
            @Override
        ...
        assertEquals(PieceType.WHITE_QUEEN, snapshot.get("F6"), "The piece on F6 must be a WHITE_QUEEN");
    }
    ...
    /**
     * Tests full branch coverage for math coordinates calculation.
     * Ensures an out-of-bounds click correctly returns null.
     */
    @Test
    void testPixelToHex_FullBranchCoverage() throws Exception {
        canvas.setWidth(1000);
        canvas.setHeight(1000);
        java.lang.reflect.Method method = HexagonCanvas.class.getDeclaredMethod("pixelToHex", double.class, double.class);
        method.setAccessible(true);
