 ...
 * <p>
 * Verifies the behavior of the canvas when no piece is currently selected.
 * It ensures that clicks on empty hexes are ignored and clicks on pieces transition
 * the canvas into the appropriate selected state.
 */
public class IdleCanvasStateTest {
    ...
    /**
     * A lightweight mock of the CanvasInterface to track method calls and state changes
     * without initializing the full JavaFX environment.
     */
    class FakeCanvas implements CanvasInterface {
        ...
        @Override public void setSelectedHex(String hex) {}
        @Override public void setDraggedPiece(PieceType piece) {}
        @Override public PieceType getPieceAt(String hex) { return PieceType.WHITE_PAWN; }
        @Override public void setMousePosition(double x, double y) {}
        @Override public void setState(CanvasInteractionState state) { this.newState = state; }
        @Override public void draw() { drawCalled = true; }
        @Override public void requestMove(String moveCommand) {}
    }
    ...
    /**
     * Ensures that clicking on a valid piece correctly grabs it and transitions
     * the canvas to the PieceSelectedState.
     */
    @Test
    void testHandleMousePressed_WithPiece() {
        dummyCanvas.simulatePieceExists = true;
        state.handleMousePressed(dummyCanvas, "F6", 100, 100);
        assertTrue(dummyCanvas.newState instanceof PieceSelectedState, "The state should transition to PieceSelectedState");
