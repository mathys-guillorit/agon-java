 ...
 *
 * <p>Verifies the behavior of the canvas when a piece is currently selected and/or dragged. Ensures
 * that moves are correctly requested, selections are cleared, and out-of-bounds drops cancel the
 * action properly.
 */
public class PieceSelectedStateTest {
  ...
  /**
   * A lightweight mock of the CanvasInterface to intercept interactions such as move requests and
   * UI visual updates.
   */
  class FakeCanvas implements CanvasInterface {
    ...
    public void requestMove(String moveCommand) {
      this.lastMoveRequested = moveCommand;
    }
  ...
  void testHandleMousePressed_NullCoordinate() {
    ...
    assertTrue(
        dummyCanvas.newState instanceof IdleCanvasState,
        "The state should revert to IdleCanvasState.");
  }
