...
public class ContestMatchTest {
  ...
  /**
   * Tests the default constructor of ContestMatch to ensure 100% line coverage. Similar to
   * SaveParser, this class acts as a static utility and its default constructor requires an
  ...
  /**
   * Verifies that the contest mode correctly executes an integration flow. It simulates a valid
   * game file, checks that the AI successfully produces a move, and validates the output format.
   */
  @Test
  ...
  /**
   * Tests the scenario where the AI is unable to find a valid move. Ensures that the system
   * correctly logs a warning or error message to the standard error stream when the game state
   * provides no options.
   */
  @Test
