...
class MctsStrategyTest {
    ...
    void testMctsRespectsTimeout() {
        ...
        assertTrue(elapsed >= timeLimit - 100, "The AI should have used its allocated time (" + elapsed + "ms)");
        assertTrue(elapsed < timeLimit + 100, "The AI should have stopped cleanly without drastically exceeding the timeout (" + elapsed + "ms)");
    ...
    void testMctsNoLegalMoves() {
        ...
        assertEquals(0, mcts.getNodeCount(), "Should not build any tree because it returns immediately");
    }
}